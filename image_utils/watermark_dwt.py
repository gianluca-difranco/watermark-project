...
# --- Funzioni Ausiliarie per il Watermark ---

def _prepare_logo_watermark(logo_path, target_shape):
    """
...
def _create_text_watermark(text, target_shape):
    """
...
# --- Funzioni Principali di Watermarking ---

def embed_watermark_dwt(original_image_path, watermark_data, output_image_path,
                        watermark_type='image', alpha=0.1, wavelet='haar'):
    """
    Incorpora un watermark (logo o testo) in un'immagine utilizzando DWT.

    :param original_image_path: Path dell'immagine host.
    :param watermark_data: Path del file logo (se type='image') o stringa di testo (se type='text').
    :param output_image_path: Path dove salvare l'immagine con watermark.
    ...
    if img is None:
        print(f"Errore: Immagine originale non trovata in {original_image_path}")
        return
    ...
    if watermark_type == 'image':
        watermark = _prepare_logo_watermark(watermark_data, target_shape)
    elif watermark_type == 'text':
        watermark = _create_text_watermark(watermark_data, target_shape)
    else:
    ...
    cv2.imwrite(output_image_path, img_w)
    print(f"Watermark incorporato con successo e salvato in: {output_image_path}")


def extract_watermark_dwt(original_image_path, watermarked_image_path,
                          output_watermark_path, alpha=0.1, wavelet='haar'):
    """
    Estrae un watermark da un'immagine (metodo NON-BLIND).

    :param original_image_path: Path dell'immagine host ORIGINALE.
    ...
    img_orig = cv2.imread(original_image_path, cv2.IMREAD_COLOR)
    ...
    # Binarizza per pulire il rumore (opzionale ma consigliato)
